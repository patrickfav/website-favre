...
export async function downloadGists(github_user, gistIds, rootDirMd, relOutDir) {
    ...
    let gistMetaData = await got.get('https://api.github.com/users/' + github_user + '/gists?per_page=100', gotHeaders)
...
async function createAndDownloadContent(gistId, gistMeta) {
    async function downloadFileContent(raw_url) {
        return await got.get(raw_url).then(response => response.body);
    }

    let markdown = "\n";

    for (const [fileName, file] of Object.entries(gistMeta.files)) {
...
function createGistFrontMatter(gistId, gistMeta, title, slug) {
    const allFiles = Object.entries(gistMeta.files).map(([k, f]) => f)
